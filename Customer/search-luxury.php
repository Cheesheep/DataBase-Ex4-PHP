...
if($condition == "designer"){//desginer的时候用select语句查询

    
    $query = "SELECT type,designer,Manufacturer,color
            ,price_perday,is_lease,luxury_id FROM luxury where $condition = '$TARGET'";  
    $res = mysqli_query($conn, $query) or die(mysqli_error($conn));

}
else{//manu的时候用存储过程来获取搜索结果
    $proc03 = "call getLuxuryInfo('$TARGET')";
    $res = mysqli_query($conn, $proc03) or die(mysqli_error($conn));
}
...
if($row){  //在这里输出整张数据表!!!
    for($i = 0 ;$i < $row ;$i++){
        ...
        if($condition == "designer"){//这里要分情况输出
